...
def generate_cnn_model_fn(model_config):
...
def train(model_config, features, train_mask, y_train, test_mask, y_test):
    ...
    #               model_config=model_config, shuffle=model_config['shuflle'])
    # # features = features/256
    # features = smooth(features, adj, model_config['smoothing'], model_config=model_config)

    model_dir = path.join(model_config['model_dir'],
                          model_config['dataset'],
                          str(model_config['train_size']),
                          model_config['name'])

    train_data = features[train_mask].astype(np.float32)
    train_labels = np.argmax(y_train[train_mask], axis=1)
    eval_data = features[test_mask].astype(np.float32)
    eval_labels = np.argmax(y_test[test_mask], axis=1)

    # Create the Estimator
    mnist_classifier = tf.estimator.Estimator(
        model_fn=generate_cnn_model_fn(model_config), model_dir=model_dir)
    ...
    while steps > 0:
        ...
        eval_input_fn = tf.estimator.inputs.numpy_input_fn(
            ...
            num_epochs=1,
            shuffle=False)
        eval_results = mnist_classifier.evaluate(input_fn=eval_input_fn, checkpoint_path=ckpt)
        print(eval_results)
    prediction =  mnist_classifier.predict(input_fn=tf.estimator.inputs.numpy_input_fn(
            x={"x": eval_data},
            num_epochs=1,
            shuffle=False))
...
if __name__ == "__main__":
    pass
    # for model_config in configuration:
    #     train(model_config)
